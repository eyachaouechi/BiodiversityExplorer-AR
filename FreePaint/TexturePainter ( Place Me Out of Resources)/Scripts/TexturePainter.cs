...
public class TexturePainter : MonoBehaviour {
	...
	[Header("Prefabs")]
	public GameObject brushEntityPrefab;
    ...
    void Update () {
		brushColor = FreePaintColorSelector.GetColor ();	//Updates our painted color with the selected color
	...
	void DoAction(){	
		...
		if(HitTestUVPosition(ref uvWorldPosition)){
			...
			if(mode==Painter_BrushMode.PAINT){
				...
				brushObj=(GameObject)Instantiate(brushEntityPrefab); //Paint a brush
    ...
    ////////////////// OPTIONAL METHODS //////////////////
     ...
     IEnumerator SaveTextureToFile(Texture2D savedTexture){		
		...
		if (currentObject == null)
			...
			string fullPath= Application.persistentDataPath + "/FreePaintedTexture";
			System.DateTime date = System.DateTime.Now;
			string fileName = currentObject.name;
			if (!System.IO.Directory.Exists(fullPath))		
				System.IO.Directory.CreateDirectory(fullPath);
			var bytes = savedTexture.EncodeToPNG();
			System.IO.File.WriteAllBytes(fullPath+ "/" + fileName + ".png", bytes);
			Debug.Log ("<color=orange>Saved Successfully!</color>"+fullPath+ "/" + fileName + ".png");
			textMeshProUGUI.text = "bravo passer au visite de parc.";
